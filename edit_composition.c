...
composition edit_composition(list_composition *list , int ID) {
    ...
    if (current != NULL){
        if (current->info.ID == ID) {
            ...
            printf ("Entrez la nouvelle date de création: \n");
            printf("le jour:");
            scanf ("%d",&current->info.date_cretion.jour);
            

            printf("le mois :");
            scanf("%d", &current->info.date_cretion.mois);
           

            printf("l'annee :");
            scanf("%d", &current->info.date_cretion.annee);
